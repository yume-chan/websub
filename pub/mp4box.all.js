			...
			debug : function(module, msg) {
				...
				if (LOG_LEVEL_DEBUG >= log_level) {
					console.debug([
						...
						"[", module, "]",
					].concat([].slice.call(arguments, 1)).join(""));
				}
			},
...
ISOFile.prototype.appendBuffer = function (ab, last) {
	...
	if (this.moov) {
		...
		if (this.onRequestMoreData) {
			this.onRequestMoreData(nextFileStart);
		}
	}
	if (this.sidx) {
...
ISOFile.cloneObject = function (input) {
	var result = {};
...
ISOFile.prototype.processSamples = function (last) {
	...
	if (this.onSamples !== null) {
		...
		for (i = 0; i < this.extractedTracks.length; i++) {
			...
			while (trak.nextSample < trak.samples.length && this.sampleProcessingStarted) {
				...
				if (sample) {
					trak.nextSample++;
					// Don't save data
					extractTrak.samples.push(ISOFile.cloneObject(sample));
		...
		if (!hasEnoughSamples && this.onRequestMoreData) {
			this.onRequestMoreData(this.nextParsePosition);
		}
	}
}
...
ISOFile.prototype.getSample = function(trak, sampleNum) {
	...
	if (index > -1) {
		...
		if (sample.size - sample.alreadyRead <= lengthAfterStart) {
			if (sample.alreadyRead === 0) {
				// The whole sample is in this buffer
				sample.data = new Uint8Array(buffer, sample.offset - buffer.fileStart, sample.size);
			} else {
			...
			this.stream.logBufferLevel();
			this.nextParsePosition = sample.offset + sample.alreadyRead;
			return null;
		}
	} else {
		this.nextParsePosition = sample.offset;
		return null;
	}
}
